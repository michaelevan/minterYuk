...
$query2 = $conn->prepare("SELECT  * FROM `mentor` where`mentor`.fk_user=:id");
$query2->execute(["id" => $id]);
foreach ($query2 as $rows) {
    $id_cust = $rows['id_mentor'];
    $nama = $rows['namalengkap_mentor'];
    $alamat = $rows['alamat_mentor'];
    $profesi = $rows['skill'];
    $telp = $rows['notelp_mentor'];
    $jk = $rows['jk_mentor'];
    $tgl = $rows['tg_lahirmentor'];
}
...
<body id="body">
        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <form action="" method="POST">
                        ...
                        <div class="form-group">
                            <label>Skill</label>
                            <input type="text" class="form-control" name="pekerjaan" value='<?php echo $profesi ?>' style="font-size:14pt;" readonly>
                        </div>
                        <div class="form-group">
                        ...
                        <div id="update" style="display:none;">
                            ...
                            <div class="col text-center">
                                Jika data yang anda isi telah benar silahkan klik submit
                                <button class="col-5 btn btn-success" type="submit" name="btnSubmit" style="font-size:14pt;">Submit</button><br>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    if (isset($_POST['btnSubmit'])) {
        ...
        if ($password == $cpassword) {
            ...
            $query5 = $conn->prepare("update mentor set namalengkap_mentor=:nama,notelp_mentor=:telp,jk_mentor=:jk,tg_lahirmentor=:tgl,alamat_mentor=:alamat 
             where fk_user=:id");
            $result = $query5->execute([
                "id" => $id, "nama" => $nama_lengkap, "telp" => $notelp, "jk" => $jenis_kelamin, "tgl" => $tanggal, "alamat" => $alamat
            ]);
